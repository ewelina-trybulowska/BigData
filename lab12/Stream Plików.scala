...
val query =streamingCountsDF.writeStream.outputMode("complete").format("memory").queryName("counting").start()
...
// MAGIC %sql select action, date_format(window.end, "MMM-dd HH:mm") as time, count from counting order by time, action
...
// MAGIC select action, date_format(window.end, "MMM-dd HH:mm") as time, count from counting order by time, action
